...
-- ################################################################
-- USING ROW_NUMBER OVER( ORDER BY SAL) - FOR EVERY DEPT
-- ################################################################

SELECT
    empno, ename, deptno, sal, ROW_NUMBER() OVER (PARTITION BY deptno ORDER BY sal)
FROM
    emp ORDER BY deptno, sal;

-- ################################################################
-- USING ROW_NUMBER OVER( ORDER BY SAL) - FOR EVERY DEPT
-- Display the second lowest value
-- It won't work because this function can only be used with SELECT clause
-- ################################################################

SELECT
    empno, ename, deptno, sal, ROW_NUMBER() OVER (PARTITION BY deptno ORDER BY sal) AS row_num
FROM
    emp 
WHERE 
    deptno = 2
